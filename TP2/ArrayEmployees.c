...
int cargarDatosEmployee(char* nombre, char *apellido, float *salario, int *sector, int sizeName,int sizeLastName)
{
    ...
    if(nombre != NULL && apellido != NULL && salario != NULL && sector != NULL)
    {
        if(!utn_getCadena("Ingrese el nombre del empleado\n","Error, nombre invalido\n",3,sizeName,3,nombre)&&
               !utn_getCadena("Ingrese el apellido del empleado\n","Error, apellido invalido\n",3,sizeName,3,apellido)&&
                !utn_getNumero(sector, "Ingrese el sector del empleado\n 100 - 200","Error, sector inexistente\n", 100, 200, 3)&&
                !utn_getNumeroFloat(salario,"Ingrese el salario\n","Error, ingrese nuevamente el salario\n",1, 99999999999.000,3))
        {
            retornoError = 1;
        }
...
int modificarEmployee(Employee* lista, int tamanio, int indice)
{
    ...
    if(lista != NULL && tamanio > 0 && indice > -1)
    {
        do
        {
            ...
            if(!utn_getNumero(&opcionMenu, "**** MODIFICAR DATOS EMPLEADO ***\n1->Nombre\n2->Apellido\n3->Salario\n4->Sector\n5->Salir","Error, opcion inexistente\n", 1, 5, 3))

            {
                ...
                {
                case 1:
                    if(utn_getCadena("Ingrese el nombre del empleado\n","Error, nombre invalido\n",3,51,3,lista[indice].name))
                    {
                        retornoError = 0;
                    }
                    break;
                case 2:
                    if(utn_getCadena("Ingrese el apellido del empleado\n","Error, apellido invalido\n",3,51,3,lista[indice].lastName))
                    {
                        retornoError = 0;
                    }
                    break;
                case 3:
                    if(!utn_getNumeroFloat(&lista[indice].salary,"Ingrese el salario\n","Error, ingrese nuevamente el salario\n",0, 999999999999999.000, 3))
                    {
                        retornoError = 0;
                    }
                    break;
                case 4:
                    if(!utn_getNumero(&lista[indice].sector, "Ingrese el sector del empleado\n 100 - 200","Error, sector inexistente\n", 100, 200, 3))
                    {
                        retornoError = 0;
                    }
                    break;
